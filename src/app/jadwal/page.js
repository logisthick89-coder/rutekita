  ...
  useEffect(() => {
    async function fetchData() {
      const [trayekRes, jadwalRes] = await Promise.all([
        ...
        supabase.from('jadwal').select('*, trayek(kode_trayek, nama_rute, aktif, jenis)').order('jam_berangkat'),
      ]);
        ...
        <div className="hidden md:flex gap-1">
          {[['/', 'Beranda'], ['/trayek', 'Trayek'], ['/jadwal', 'Jadwal'], ['/tarif', 'Tarif'], ['/peta', 'Peta']].map(([href, label]) => (
            <Link key={label} href={href}
              ...
              ${label === 'Jadwal' ? 'bg-violet-500/20 text-violet-300 font-semibold' : 'text-gray-400 hover:text-white hover:bg-white/10'}`}>
              {label}
            </Link>
          ))}
        </div>
      ...
      <div className="fixed bottom-0 left-0 right-0 md:hidden bg-[#0f0f1a]/95 border-t border-white/10 flex h-14 z-40">
        {[['/', '🏠', 'Beranda'], ['/trayek', '🚌', 'Trayek'], ['/peta', '🗺️', 'Peta'], ['/jadwal', '📅', 'Jadwal']].map(([href, icon, label]) => (
          <Link key={label} href={href}
            ...
            ${label === 'Jadwal' ? 'text-violet-400' : 'text-gray-500 hover:text-white'}`}>
